...
public class YiHanziToPinyin
{
	...
	private static YiHanziToPinyin sInstance;
	...
	protected YiHanziToPinyin(boolean hasChinaCollator)
	{
		mHasChinaCollator = hasChinaCollator;
	}

	public static YiHanziToPinyin getInstance()
	{
		synchronized (YiHanziToPinyin.class)
		{
			if (sInstance != null)
			{
				return sInstance;
			...
			for (int i = 0; i < locale.length; i++)
			{
				if (locale[i].equals(Locale.CHINA))
				{
					...
					sInstance = new YiHanziToPinyin(true);
					return sInstance;
				}
			}
			Log.w(TAG,
					"There is no Chinese collator, HanziToPinyin is disabled");
			sInstance = new YiHanziToPinyin(false);
			return sInstance;
		}
	}
	...
	{
		ArrayList<Token> tokens = getInstance().get(input);
